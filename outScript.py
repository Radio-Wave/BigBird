...
def stream_elevenlabs_to_blackhole(api_key: str | None,
                                   voice_id: str,
                                   text: str,
                                   ...
                                   jitter_ms: int = 150,
                                   output_format: str = "pcm_44100") -> bool:
    try:
        api_key = api_key or os.getenv("ELEVEN_API_KEY") or os.getenv("ELEVENLABS_API_KEY")
        if not api_key:
            raise RuntimeError("Set ELEVEN_API_KEY (or ELEVENLABS_API_KEY) in env or pass api_key explicitly")

        url = API_URL_TMPL.format(voice_id=voice_id)
        requested_of = output_format
        ...
        print(f"[ERROR] stream_elevenlabs_to_blackhole: {e}")
        return False

def streamToSpeakers(api_key: str, voice_id: str, text: str,
                     *,
                     model_id: str = "eleven_v3",
                     device_hint: str = "BlackHole",
                     device_rate: int = 44100,
                     ...
                     jitter_ms: int = 150,
                     output_format: str = "pcm_44100") -> bool:
    """Convenience wrapper so BigBirdApi can call this directly.
    Returns True on success, False on error.
    """
    return stream_elevenlabs_to_blackhole(api_key, voice_id, text,
                                          model_id=model_id,
...
def main():
    ...
    ok = stream_elevenlabs_to_blackhole(
        api_key=api_key,
...
__all__ = ["streamToSpeakers", "stream_elevenlabs_to_blackhole"]